...
$db->query("INSERT INTO apps_reverse_response (api_id, reverse_response_content, response_time) VALUES ('15', '".$content."', NOW())");

if(isset($_GET['status']) && $_GET['status']!='' && isset($_GET['tranId']) && $_GET['tranId']!='') {
	
	$api_status = trim(mysql_real_escape_string($_GET['status']));

	$recharge_id = trim(mysql_real_escape_string($_GET['tranId']));

	
	$recharge_info=$db->queryUniqueObject("SELECT * FROM apps_recharge WHERE recharge_id='".$recharge_id."' AND api_id='15' ");
	if($recharge_info) {
		//operator row
		...
		if($agent_info) {
			...
			if($api_status=='SUCCESS') {
				...
				$operator_ref_no=trim(mysql_real_escape_string($_GET['opTId']));
				...
				$db->execute("UPDATE apps_recharge SET status='".$status."',$status_update='".$status."', status_details='".$status_details."', api_status='', api_status_details='', response_date=NOW(), operator_ref_no='".$operator_ref_no."', is_callback='1' WHERE recharge_id='".$recharge_info->recharge_id."' ");
