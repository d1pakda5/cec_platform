...
include("../config.php");
$rows = [];		
$query = $db->query("SELECT * FROM apps_recharge WHERE api_id = '1' AND status IN (0,1) AND operator_ref_no = '' ORDER BY request_date DESC LIMIT 25");
...
foreach($rows as $row) {
	...
	if($api_status == '0' && $operator_ref_no != '') {
		...
		if(strlen($operator_ref_no) > 4 && strlen($operator_ref_no) < 24) {
	...
	echo $row->request_txn_no.", Time: ".$row->request_date.", Status: ".$status.", Status Details: ".$status_details.", Operator Ref No: ".$operator_ref_no."<br>";
