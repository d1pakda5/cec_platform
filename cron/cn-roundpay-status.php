...
include("../config.php");
$rows = [];		
$query = $db->query("SELECT * FROM apps_recharge WHERE api_id='6' AND status='1' AND (request_date < DATE_SUB(NOW(), INTERVAL 30 MINUTE)) ORDER BY request_date DESC LIMIT 100");
...
foreach($rows as $row) {
	...
	$url = "http://roundpay.in/API/APIService.aspx?userid=".$roundpay['userid']."&pass=".$roundpay['pass']."&csagentid=".$row->recharge_id;
