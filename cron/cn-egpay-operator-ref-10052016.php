<?php
include("../config.php");
$query = $db->query("SELECT * FROM apps_recharge WHERE api_id = '1' AND status IN (0,1) AND operator_ref_no = '' ORDER BY request_date DESC LIMIT 5");
while($result = $db->fetchNextObject($query)) {	
	$operator_ref_no = $result->operator_ref_no;
	
	$url = "http://api.egpay.in/GetUpdatedStatus.aspx?UID=".$eg_pay['uid']."&UPASS=".$eg_pay['pass']."&SUBUID=".$eg_pay['subuid']."&SUBUPASS=".$eg_pay['subupass']."&TXNNO=".$result->request_txn_no;
	...
	if($api_status == '0' && $operator_ref_no != '') {
		...
		$db->execute("UPDATE apps_recharge SET status = '".$status."', status_details = '".$status_details."', operator_ref_no = '".$operator_ref_no."' WHERE recharge_id = '".$result->recharge_id."' ");
	}
