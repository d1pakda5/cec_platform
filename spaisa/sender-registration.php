...
if(strpos($checkExplode['6'], 'ERROR=0')!==false) {
	...
	if(strpos($payExplode['6'], 'ERROR=0')!==false) {
		...
		if($jsFetch['CardExists']=='N') {
			...
			$jsDmtResponse = '{"ResponseCode":"0","Message":"Verification Code for your wallet, send on requested mobile Number","RequestNo":"'.$jsFetch['RequestNo'].'","MobileNo":"'.$jsFetch['MobileNo'].'","SenderRegistered":"'.$jsFetch['CardExists'].'"}';
						
		} else {
			
			$jsDmtResponse = '{"ResponseCode":"1","Message":"Sender is already registered","MobileNo":"'.$account.'"}';
