...
$db->execute("INSERT INTO `mt_logs`(`mt_logs_id`, `mt_logs_date`, `mt_logs_type`, `mt_logs`) VALUES ('', NOW(), 'DMR PRE CHECK', '".$signInResult[1]."::".$cpUrl['check']."')");	

$checkOutput = $sp->cpRequest($signInResult[1], $cpUrl['check']);
$checkExplode = explode("\n",$checkOutput);	

$db->execute("INSERT INTO `mt_logs`(`mt_logs_id`, `mt_logs_date`, `mt_logs_type`, `mt_logs`) VALUES ('', NOW(), 'DMR RSP CHECK', '".$checkOutput."')");	
...
if($dmt_check_error=='0' && $dmt_check_result=='0') {	
	...
	$db->execute("INSERT INTO `mt_logs`(`mt_logs_id`, `mt_logs_date`, `mt_logs_type`, `mt_logs`) VALUES ('', NOW(), 'DMR PRE PAY', '".$signInResult[1]."::".$cpUrl['pay']."')");	
	
	$payOutput = $sp->cpRequest($signInResult[1], $cpUrl['pay']);
	$payExplode = explode("\n",$payOutput);
	
	$db->execute("INSERT INTO `mt_logs`(`mt_logs_id`, `mt_logs_date`, `mt_logs_type`, `mt_logs`) VALUES ('', NOW(), 'DMR RSP PAY', '".mysql_real_escape_string($payOutput)."')");	
	...
	if($dmt_pay_error=='0' && $dmt_pay_result=='0') {
		...
		$jsDmtResponse = '{"ResponseCode":"0","Message":"Remittance for beneficary is successful.","RequestNo":"'.$jsFetch['RequestNo'].'","MobileNo":"'.$jsFetch['MobileNo'].'","MoneyRemittance":'.$jsMoneyRemittance.',"Beneficiary":'.$jsBeneficiary.',"Recharge":'.$jsRecharge.'}';	
		$status = "0";
