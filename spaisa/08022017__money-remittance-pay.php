...
$db->execute("INSERT INTO `mt_logs`(`mt_logs_id`, `mt_logs_date`, `mt_logs_type`, `mt_logs`) VALUES ('', NOW(), 'REMITTANCE PRE PAY', '".$signInResult[1]."::".$cpUrl['pay']."')");	

$payOutput = $sp->cpRequest($signInResult[1], $cpUrl['pay']);
$payExplode = explode("\n",$payOutput);

$db->execute("INSERT INTO `mt_logs`(`mt_logs_id`, `mt_logs_date`, `mt_logs_type`, `mt_logs`) VALUES ('', NOW(), 'REMITTANCE PAY', '".$payOutput."')");	
...
if($dmt_pay_error=='0' && $dmt_pay_result=='0') {
	...
	$jsDmtResponse = '{"ResponseCode":"18","Message":"Invalid response ('.$dmt_pay_error.' | '.$dmt_pay_result.') '.$dmt_value_errmsg.'"}';	
	$status = "2";
	$status_details = 'Transaction Failed';
