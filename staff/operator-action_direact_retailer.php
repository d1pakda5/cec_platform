...
include("../config.php");
//include("common.php");
if(!isset($_SESSION['staff'])) header("location:index.php");
...
if(isset($_GET['id']) && $_GET['id'] != '' && isset($_GET['status']) && $_GET['status'] != '') {
	...
	if($action == '1') {
		$db->execute("UPDATE usercommissions SET status = 1 WHERE id = '".$request_id."' ");
	} else if($action == '0') {
		$db->execute("UPDATE usercommissions SET status = 0 WHERE id = '".$request_id."' ");
