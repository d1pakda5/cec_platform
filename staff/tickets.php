...
$sWhere = "WHERE ticket.submit_date BETWEEN '".$aFrom."' AND '".$aTo."'";

if(isset($_GET['s']) && $_GET['s'] != '') {
	$sWhere .= " AND ( ticket.title LIKE '%".mysql_real_escape_string($_GET['s'])."%' ) ";
}
if(isset($_GET['u']) && $_GET['u'] != '') {
	$sWhere .= " AND ticket.urgency = '".mysql_real_escape_string($_GET['u'])."' ";
}
if(isset($_GET['status']) && $_GET['status'] != '') {
	$sWhere .= " AND ticket.status = '".mysql_real_escape_string($_GET['status'])."' ";
}
$statement = "tickets ticket LEFT JOIN apps_user user ON ticket.submited_by = user.uid $sWhere ORDER BY ticket.last_reply_date,ticket.submit_date DESC";
//Pagination
				...
				<table class="table">
					<thead>
						<tr>
							...
							<th>Subject</th>
							<th>User</th>
							<th width="1%">Urgency</th>
							<th width="1%">Status</th>
						</tr>
					</thead>
					<tbody>
						<?php
						$query = $db->query("SELECT ticket.*, user.company_name FROM {$statement} LIMIT {$startpoint}, {$limit}");
