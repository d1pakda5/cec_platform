...
include('../config.php');
include('common.php');
if(empty($sP['reports']['apiresponse'])) { 
...
if(isset($_GET['s']) && $_GET['s'] != '') {
	$sWhere .= " AND ( log.txn_no LIKE '%".mysql_real_escape_string($_GET['s'])."%' OR log.http_response_content LIKE '%".mysql_real_escape_string($_GET['s'])."%' )  ";
} else {
	$sWhere .= " AND log.update_date BETWEEN '".$aFrom."' AND '".$aTo."' ";
}
if(isset($_GET['api']) && $_GET['api'] != '') {
	$sWhere .= " AND log.api_id = '".mysql_real_escape_string($_GET['api'])."' ";
}

$statement = "apps_response_log log LEFT JOIN api_list api ON log.api_id = api.api_id $sWhere ORDER BY log.update_date DESC";
...
$self = $tbl->remove_page_param('rpt-api-response.php');
$meta['title'] = "Recharge Response Reports";
include('header.php');
?>
<script>
...
<div class="content">
	<div class="container-fluid">
		<div class="page-header">
			<div class="page-title">Reports <small>/ Recharge Response</small></div>
		</div>
		<div class="box">
			<div class="box-header">
				<h3 class="box-title"><i class="fa fa-list"></i> List Response</h3>
			</div>			
				...
				<table class="table table-basic">
					...
					<tbody>
						...
						<tr>
							...
							<td><?php echo $result->http_response_code;?></td>
							<td><?php echo $result->api_name;?></td>
							<td><?php echo $result->http_response_content;?></td>
						</tr>
						<?php } ?>
					</tbody>
				</table>
