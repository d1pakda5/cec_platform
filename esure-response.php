...
$db->query("INSERT INTO apps_reverse_response (api_id, reverse_response_content, response_time) VALUES ('16', '".$content."', NOW())");
...
if(isset($_GET['status']) && $_GET['status']!='' && isset($_GET['rpid']) && $_GET['rpid']!='' && isset($_GET['mobile']) && $_GET['mobile']!='' && isset($_GET['amount']) && $_GET['amount']!='') {
	...
	$recharge_info=$db->queryUniqueObject("SELECT * FROM apps_recharge WHERE account_no='".$account."' AND amount='".$amount."' AND recharge_id='".$recharge_id."' AND api_id='16' ");
	if($recharge_info) {
		//operator row
		...
		if($agent_info) {
			...
			if($api_status=='SUCCESS') {
				...
				$account_id2=$recharge_info->offline_uid;
				 ...
				 if($recharge_count>5 && $start_with)
				 {
