...
if(isset($_GET['id']) && $_GET['id'] != '' && isset($_GET['action']) && $_GET['action'] != '') {
	...
	if($action == 'activate') {
		$db->execute("UPDATE service_type SET status = '1' WHERE service_type_id = '".$request_id."' ");
	} else if($action == 'suspend') {
		$db->execute("UPDATE service_type SET status = '0' WHERE service_type_id = '".$request_id."' ");
	}	
