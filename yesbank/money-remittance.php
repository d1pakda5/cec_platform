    					...
    					if($isfund && (($isfund->balance - $isfund->cuttoff) >= ($amount + $sCommission['samount']))) {
    						...
    						if(isset($ybCheckStatus) && $ybCheckStatus=="0") {
    							...
    							if($wallet && $ts1) {
    								...
    								if($commit) {
    									$remark = "YB-DMT Remittance: $recharge_id, $account, $amount, $debit_amount";
    									...
    									//Send to payment api
    									include(DIR."/yesbank/money-remittance-pay.php");
    									...
    									elseif($status=='2') {										
    										...
    										if($wallet1 && $ts4) {													
    											$commit = $db->query("COMMIT");	
    											if($commit) {													
    												$remark_new = "YB-DMT REVERT: $recharge_id, $account, $amount, $debit_amount, failed revert";
