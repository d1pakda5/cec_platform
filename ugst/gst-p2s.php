...
$invoice = $db->queryUniqueObject("SELECT * FROM gst_invoices WHERE uid='".$uid."' AND invoice_month='".$invoice_month_ymd."' AND bill_type='".$type."' ");
if($invoice) {
...
<div class="header">
	<div class="container-fluid text-right">
		...
		<div class="btn-group btn-group-sm hidden-print">
			...
			<a href="pdf.php?id=<?php echo $invoiceid;?>&type=<?php echo $type;?>" class="btn btn-default"><i class="fa fa-download"></i> Download</a>
		</div>
		<?php }else{ ?>
		<div class="btn-group btn-group-sm hidden-print">
	...
	<div class="container">
		<div id="divPrint">
			...
			<table class="table table-bordered">
				...
				<?php
				$a_statement = "SELECT SUM(rch_amount) AS amount, SUM(rch_comm_value) AS comm FROM gst_monthly_txns $sWhere AND item_group='4'";
				$a = $gst->getItemP2S($a_statement,'4');
				?>
				<tr>
					<td>1.
				...
				<?php
				$b_statement = "SELECT SUM(rch_amount) AS amount, SUM(rch_comm_value) AS comm FROM gst_monthly_txns $sWhere AND item_group='5'";
				$b = $gst->getItemP2S($b_statement,'5');
				?>			
				<tr>
					<td>2.
						<input type="hidden" name="item_id[]" id="item_id" value="2">
						<input type="hidden" name="item_desc[]" id="item_desc" value="<?php echo $b['desc'];?>">
						<input type="hidden" name="item_sac[]" id="item_sac" value="<?php echo $b['sac'];?>">
						<input type="hidden" name="item_amt[]" id="item_amt" value="<?php echo $b['amount'];?>">
						<input type="hidden" name="item_net[]" id="item_net" value="<?php echo $b['net'];?>">
						<input type="hidden" name="item_taxable[]" id="item_taxable" value="<?php echo $b['taxable'];?>">
						<input type="hidden" name="item_tax[]" id="item_tax" value="<?php echo $b['gst'];?>">
						<input type="hidden" name="item_rate[]" id="item_rate" value="<?php echo $b['rate'];?>">
						<input type="hidden" name="item_unit[]" id="item_unit" value="Unit">
					</td>
					<td><?php echo $b['desc'];?></td>
					<td><?php echo $b['sac'];?></td>
					<td align="right"></td>
					<td></td>
					<td></td>
					<td align="right" class="tot_netamt"><?php echo $b['taxable'];?></td>
				</tr>
				<?php
				$total_amount = $a['amount'] + $b['amount'];
				$total_net = $a['net'] + $b['net'];
				$total_taxable = $a['taxable'] + $b['taxable'];
			...
			<table class="table table-bordered">
				...
				<?php
				$a_gst = $gst->getTaxSummary($a['net'],$user->gst_type);
				$a_tax_total = $a_gst['cgst_amount']+$a_gst['sgst_amount']+$a_gst['igst_amount'];
				?>
				<tr>
					<td><?php echo $a['sac'];?></td>
					<td align="right"><?php echo $a['taxable'];?></td>
					<td><?php echo $a_gst['cgst_rate'];?></td>
					<td align="right"><?php echo $a_gst['cgst_amount'];?></td>
					<td><?php echo $a_gst['sgst_rate'];?></td>
					<td align="right"><?php echo $a_gst['sgst_amount'];?></td>
					<td><?php echo $a_gst['igst_rate'];?></td>
					<td align="right"><?php echo $a_gst['igst_amount'];?></td>
					<td align="right"><?php echo $a_tax_total;?></td>
				</tr>				
				<?php
				$b_gst = $gst->getTaxSummary($b['net'],$user->gst_type);
				$b_tax_total = $b_gst['cgst_amount']+$b_gst['sgst_amount']+$b_gst['igst_amount'];
				?>
				<tr>
					<td><?php echo $b['sac'];?></td>
					<td align="right"><?php echo $b['taxable'];?></td>
					<td><?php echo $b_gst['cgst_rate'];?></td>
					<td align="right"><?php echo $b_gst['cgst_amount'];?></td>
					<td><?php echo $b_gst['sgst_rate'];?></td>
					<td align="right"><?php echo $b_gst['sgst_amount'];?></td>
					<td><?php echo $b_gst['igst_rate'];?></td>
					<td align="right"><?php echo $b_gst['igst_amount'];?></td>
