<?php
	/*
	* Recharge A2Z Recharge API 
	*/
	$request_txn_no = $recharge_id;
	
	$url = "http://rechargea2z.com/API/APIService.aspx?userid=".$rechargea2z['userid']."&pass=".$rechargea2z['pass']."&mob=".$account."&opt=".$operator_info->code_rechargeatz."&amt=".$amount."&agentid=".$request_txn_no."&fmt=Json";
	...
	if($output) {
		...
		if($api_status == 'SUCCESS') {		
			...
			if(preg_match("/Failed/i", $api_status_details)) {
				$status = '2';
				$status_details = 'Transaction Failed';	
			} else if(preg_match("/Success/i", $api_status_details)) {		
				$status = '0';
				$status_details = 'Transaction Successful';
			}
			else {		
