...
if($operator_info) {
	$duplicate_info = $db->queryUniqueObject("SELECT recharge_id FROM apps_recharge WHERE account_no = '".$account."' AND amount = '".$amount."' AND operator_id = '".trim($operator_info->operator_id)."' AND status NOT IN (1) AND (request_date > DATE_SUB(NOW(), INTERVAL 2 HOUR)) ORDER BY recharge_id DESC ");
	if($duplicate_info) {
