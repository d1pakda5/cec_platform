...
function getUserGstTxnsOlds($user,$bill_type,$amount,$is_surcharge='0') {
	$net_comm = 0;
	$tax_comm = 0;
	$tot_deduct_com = 0;
	$gst_amount = 0;
...
function getGstTxnsForOld($user,$bill_type,$amount,$is_surcharge='0') {
	$net_comm = 0;
	$tax_comm = 0;
	$tot_deduct_com = 0;
	$gst_amount = 0;
...
function getGstTxnsForDist($bill_type,$amount,$is_surcharge='0') {
	$net_comm = 0;
	$tax_comm = 0;
	$tot_deduct_com = 0;
	$gst_amount = 0;
	...
	if($row) {
		if($row->is_surcharge=='y') {
			if($row->is_percentage=='y') {				
				$result = array('dsPer'=>$row->comm_dist, 'dsCom'=>$amount*$row->comm_dist/100, 'rtPer'=>$row->comm_api, 'rtCom'=>$amount*$row->comm_api/100, 'surcharge'=>'y', 'samount'=>$amount*$row->surcharge_value/100);
			} else {
				$result = array('dsPer'=>'0', 'dsCom'=>$row->comm_dist, 'rsPer'=>'0', 'rtCom'=>$row->comm_api, 'surcharge'=>'y', 'samount'=>$row->surcharge_value);
			}
		} else {
			$result = array('dsPer'=>$row->comm_dist, 'dsCom'=>$amount*$row->comm_dist/100, 'rtPer'=>$row->comm_api, 'rtCom'=>$amount*$row->comm_api/100, 'surcharge'=>'n', 'samount'=>'0');
		}
	} else {
		$result = array('dsPer'=>'0', 'dsCom'=>'0', 'rtPer'=>'0', 'rtCom'=>'0', 'surcharge'=>'n', 'samount'=>'0');
	}
	return $result;
}

function getApiCommission($uid,$operatorid,$amount,$bill_type) {
	global $db;
	$row = $db->queryUniqueObject("SELECT * FROM usercommissions WHERE uid='".$uid."' AND operator_id='".$operatorid."' ");
	if($row) {
		if($bill_type=='3') {
...
function getRetCommission($uid,$operatorid,$amount,$bill_type) {
	global $db;
	$row = $db->queryUniqueObject("SELECT * FROM retcommissions WHERE operator_id='".$operatorid."' ");
	if($row) {
		if($bill_type=='3') {
			$result = array('dsPer'=>'0', 'dsCom'=>$row->comm_dist, 'rtPer'=>'0', 'rtCom'=>$row->comm_ret, 'surcharge'=>'y', 'samount'=>$row->surcharge_value);
		} else {
			$result = array('dsPer'=>$row->comm_dist, 'dsCom'=>$amount*$row->comm_dist/100, 'rtPer'=>$row->comm_ret, 'rtCom'=>$amount*$row->comm_ret/100, 'surcharge'=>'n', 'samount'=>'0');
		}
	} else {
		$result = array('dsPer'=>'0', 'dsCom'=>'0', 'rtPer'=>'0', 'rtCom'=>'0', 'surcharge'=>'n', 'samount'=>'0');
	}
...
function getDistCommission($uid,$operatorid,$amount,$bill_type) {
	global $db;
	$row = $db->queryUniqueObject("SELECT * FROM retcommissions WHERE operator_id='".$operatorid."' ");
	if($row) {
		if($bill_type=='3') {
...
function getUsersCommissionNew($uid,$operatorid,$amount,$api='') {
	...
	if($row) {		
		if($api=='api') {
			...
			$result = array('dsCom'=>$comm_dist, 'rtCom'=>$comm_api, 'surcharge'=>$row->is_surcharge, 'samount'=>$samount);			
		} else {			
			if($row->commission_type=='p') {
				$comm_dist = $amount*$row->comm_dist/100;
			...
			$result = array('dsCom'=>$comm_dist, 'rtCom'=>$comm_ret, 'surcharge'=>$row->is_surcharge, 'samount'=>$samount);
		}		
	} else {
		...
		if($api=='api') {
			...
			$result = array('dsCom'=>$comm_dist, 'rtCom'=>$comm_ret, 'surcharge'=>$row2->is_surcharge, 'samount'=>$samount);	
		}		
	}
	return $result;
}
