	...
	if($recharge_info->is_refunded == 'n') {
		...
		if($recharge_user) {
			...
			if($operator_ref_no !='' && strlen($operator_ref_no) >= '4' && preg_match('/[0-9]/', $operator_ref_no)) {
				...
				smsSendSingle($user_info->mobile, $message, 'complaint_register');
				exit();
			} else {
				$trans_info = $db->queryUniqueObject("SELECT * FROM transactions WHERE transaction_ref_no = '".$msg_param."' ORDER BY transaction_date DESC");
				if($trans_info) {
					if($trans_info->type == 'dr') {						
						...
						if($complaint) {
							...
							if($recharge_info->request_date > $prev_date) {
								...
								$complaint_id = $db->lastInsertedId();
								$db->execute("UPDATE `apps_recharge` SET `is_complaint`='y' WHERE recharge_id = '".$recharge_info->recharge_id."' ");
								$message = "Complaint Registered successfully, Txn:".$msg_param.". Complaint Id:".$complaint_id;
								smsSendSingle($user_info->mobile, $message, 'complaint_register');
								exit();
							} else {
						...
						$message = "Recharge already refunded x, Txn:".$msg_param;
						smsSendSingle($user_info->mobile, $message, 'complaint_register');
						exit();
					}
				} else {
		...
		$message = "Recharge already refunded y, Txn:".$msg_param;
		smsSendSingle($user_info->mobile, $message, 'complaint_register');
		exit();
	}
} else {
