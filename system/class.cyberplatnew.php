...
class CyberPlat {
	function cyberplatUrl($operator) {
		if($operator == 'RC') {
			...
			//$cp_url = array(
				//'check'=>'https://in.cyberplat.com/cgi-bin/ts/ts_pay_check.cgi/t=249',
				//'pay'=>'https://in.cyberplat.com/cgi-bin/ts/ts_pay.cgi/t=249',
				//'status'=>'https://in.cyberplat.com/cgi-bin/ts/ts_pay_status.cgi'
			//);
		} else if($operator == 'BC') {
			//AIRCEL POSTPAID
	...
	function cyberplatRequest($qs,$url,$param=''){		
		$urln = $url."?inputmessage=".urlencode($qs).$param;
		$opts = array( 
			'http'=>array( 
			'method'=>"GET", 
