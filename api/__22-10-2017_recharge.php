...
if($agent_info->uid=='20000004' || $agent_info->uid=='20030118' || $agent_info->uid=='20030133') {
	if($api_id=='1' || $api_id=='10') {
		if($status=='0' || $status=='2') {
			if($user_api_info->reverse_url!='') {	
				...
				curl_close($ch);
				$db->execute("INSERT INTO `reverse_url_log`(`log_id`, `client_id`, `api_id`, `url_detail`, `status_code`, added_date) VALUES ('', '".$agent_info->uid."', '".$api_id."', '".$hitUrl."', '".$http_code."', NOW())");
