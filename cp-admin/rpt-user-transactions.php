...
if(isset($_GET['s']) && $_GET['s']!='') {
	$sWhere = " WHERE (trans.remark LIKE '%".mysql_real_escape_string($_GET['s'])."%' OR trans.transaction_ref_no LIKE '%".mysql_real_escape_string($_GET['s'])."%' OR trans.amount LIKE '%".mysql_real_escape_string($_GET['s'])."%' OR user.company_name LIKE '%".mysql_real_escape_string($_GET['s'])."%') ";
} else {
...
<!--								<button type="button" onclick="doExcel1()" class="btn btn-success"><i class="fa fa-file-excel-o"></i> Excel-1</button>
				...
				<table class="table table-condensed table-basic table-bordered table-striped">
					<thead>
						<tr>
							<th width="5%">S.No.</th>
							<th>Date</th>
					...
					<tbody>
						<?php
						$scnt = 1;
						$query = $db->query("SELECT trans.*, user.company_name,adm.fullname, user.uid FROM {$statement} LIMIT {$startpoint}, {$limit} ");
					...
					<tfoot>
						<?php $qry3 = $db->query("SELECT SUM(trans.amount) AS totaltra FROM transactions trans LEFT JOIN apps_user user ON trans.to_account_id = user.uid  LEFT JOIN apps_admin adm on user.assign_id=adm.admin_id  $sWhere and trans.type='dr' ORDER BY trans.transaction_date DESC");
						$row3 = $db->fetchNextObject($qry3); ?>
						<tr>
							<td align="right" colspan="7"><b>Total</b></td>
							<td align="right"><b class="text-red"><?php echo round($row3->totaltra,2);?></b></td>
						
						<?php $qry4 = $db->query("SELECT SUM(trans.amount) AS totaltra FROM transactions trans LEFT JOIN apps_user user ON trans.to_account_id = user.uid  LEFT JOIN apps_admin adm on user.assign_id=adm.admin_id  $sWhere and trans.type='cr' ORDER BY trans.transaction_date DESC");
						$row4 = $db->fetchNextObject($qry4); ?>
						
							<td align="right"><b class="text-red"><?php echo round($row4->totaltra,2);?></b></td>
						</tr>
					</tfoot>
				</table>
			</div>
