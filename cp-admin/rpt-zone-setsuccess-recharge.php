...
if(isset($_GET['s']) && $_GET['s'] != '') {
	...
	$sWhere .= " AND rlog.updated_date BETWEEN '".$aFrom."' AND '".$aTo."' ";
}
...
$statement = "recharge_change_log rlog LEFT JOIN apps_recharge recharge ON rlog.recharge_id = recharge.recharge_id LEFT JOIN operators opr ON recharge.operator_id = opr.operator_id LEFT JOIN apps_user user ON recharge.uid = user.uid $sWhere ORDER BY recharge.request_date DESC";
...
$self = $tbl->remove_page_param('rpt-zone-setsuccess-recharge.php');
		...
		<div class="box">
			...
			<div class="box-body no-padding min-height-480">
				...
				<table class="table table-striped table-condensed-sm">
					...
					<tbody>
						<?php
						$query = $db->query("SELECT rlog.*, recharge.*, opr.operator_name, user.company_name FROM {$statement} LIMIT {$startpoint}, {$limit} ");
