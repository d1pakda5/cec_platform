...
$statement = "apps_recharge rch LEFT JOIN operators opr ON rch.operator_id=opr.operator_id LEFT JOIN apps_user usr ON rch.uid=usr.uid LEFT JOIN usercommissions ucm ON usr.dist_id=ucm.uid AND rch.operator_id=ucm.operator_id $sWhere ORDER BY rch.request_date ASC";
...
$self = $tbl->remove_page_param('rpt-retailer-gst-recharge-wise.php');
...
function doExcels(){
	...
	var type = $('#type').val();
	var uid = $('#uid').val();
	window.location = 'excel/retailer-gst-recharge-wise.php?from='+from+'&to='+to+'&type='+type+'&uid='+uid;
}
</script>
		...
		<div class="box">
			...
			<div class="box-body no-padding min-height-480">
				...
				<table class="table table-condensed table-striped table-bordered">
					...
					<tbody>
						<?php
						$query = $db->query("SELECT rch.*, opr.operator_name, opr.billing_type, opr.is_surcharge, usr.user_type, usr.company_name, usr.states, ucm.comm_ret, ucm.comm_api FROM {$statement} LIMIT {$startpoint}, {$limit} ");
