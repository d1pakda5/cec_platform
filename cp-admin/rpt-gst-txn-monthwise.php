...
if(isset($_GET["inv"]) && $_GET["inv"]!='') {
	$sWhere .= " AND user.gst_invoice='".mysql_real_escape_string($_GET["inv"])."' ";
}

$statement = "gst_monthlytxn gst LEFT JOIN gst_invoice_user user ON gst.uid=user.uid $sWhere ORDER BY gst.rch_date ASC";
...
$self = $tbl->remove_page_param('rpt-gst-txn-monthwise.php');
//
		...
		<div class="box">
			...
			<div class="box-body no-padding min-height-480">
				...
				<table class="table table-condensed table-striped table-bordered">
					...
					<thead>
						<?php $qry1 = $db->query("SELECT SUM(rch_amount) AS sum_amount, SUM(rch_comm_value) AS sum_comm_value, SUM(gst_net) AS sum_gst_net, SUM(gst_tax) AS sum_gst_tax, SUM(taxable_value) AS sum_taxable_value FROM {$statement}");
					...
					<tfoot>
						<?php $qry = $db->query("SELECT SUM(rch_amount) AS sum_amount, SUM(rch_comm_value) AS sum_comm_value, SUM(gst_net) AS sum_gst_net, SUM(gst_tax) AS sum_gst_tax, SUM(taxable_value) AS sum_taxable_value FROM {$statement}");
