...
$sWhere = "WHERE user.user_type='5' AND user.status='1' ";
...
if(isset($_POST['bulkaction'])) {
	...
	header("location:retailer-trans.php?msg=success&paged=".$paged);
	exit();
}
				...
				<table class="table table-bordered table-condensed">
					...
					<tbody>
						...
						while($result = $db->fetchNextObject($query)) {
							$trans_done = "N";
							$trans = $db->queryUniqueObject("SELECT * FROM transactions WHERE account_id='".$result->uid."' AND transaction_date BETWEEN '".$aFrom."' AND '".$aTo."' ORDER BY transaction_date DESC ");
							if($trans) {
								$trans_done = "Y";
							}						
						?>
						<tr<?php if($trans_done=='N') {?> class="bg-danger"<?php } ?>>
							<td align="center"><?php echo $scnt++;?></td>
							<td align="center"><input type="checkbox" name="uid[<?php echo $result->uid;?>]" value="<?php echo $result->uid;?>" <?php if($trans_done=='N') {?>checked="checked"<?php } ?> /></td>
