...
//echo $aFrom." ".$aTo;
$sWhere = "WHERE trans.transaction_date >= '".$aFrom."' AND trans.transaction_date <= '".$aTo."' AND trans.type='dr' AND user.user_type='5' AND user.status='1' ";
...
$statement = "transactions trans LEFT JOIN apps_user user ON trans.account_id=user.uid $sWhere GROUP BY user.uid ORDER BY trans.transaction_date DESC";
...
$self = $tbl->remove_page_param('recharge-trans1.php');
...
if(isset($_POST['bulkaction'])) {
	...
	header("location:retailer-trans1.php?msg=success&paged=".$paged);
	exit();
}
		...
		<div class="box">
			...
			<div class="box-body no-padding min-height-480">
				...
				<table class="table table-bordered table-condensed">
					...
					<tbody>
						<?php						
						$query = $db->query("SELECT trans.*, user.uid, user.user_id, user.user_type, user.fullname, user.company_name, user.mobile, user.username, user.status, user.added_date FROM {$statement} LIMIT {$startpoint}, {$limit} ");
						if($db->numRows($query) < 1) echo "<tr><td colspan='100%'>No Result Found</td></tr>";
						while($result = $db->fetchNextObject($query)) {
							//$wallet = $db->queryUniqueObject("SELECT * FROM apps_wallet WHERE uid='".$result->uid."' ");
						?>
						<tr>
							<td align="center"><?php echo $scnt++;?></td>
							<td align="center"><input type="checkbox" name="uid[<?php echo $result->uid;?>]" /></td>
							...
							<td align="right"><b class="text-primary"></b></td>	
							<td align="right"><b class="text-primary"></b></td>
							<td align="right"><?php echo date("d-m-Y", strtotime($result->transaction_date));?></td>
							<td align="center">
