...
if($_POST['recharge_id'] == '' || $_POST['action'] == '') {
	...
	if($complaint_info) {	
		...
		if($status == '1') {			
			if($complaint_info->is_refunded == 'y') {
				...
				if($trans_info) {
					if($trans_info->type == 'dr') {
						...
						$remark = "REFUND: $recharge_id, $operator_info->operator_name, $amount, $complaint_id, $remark";
						$db->execute("INSERT INTO `transactions`(`transaction_id`, `transaction_date`, `account_id`, `to_account_id`, `type`, `amount`, `closing_balance`, `transaction_term`, `transaction_ref_no`, `remark`, `transaction_user_type`, `transaction_by`) VALUES ('', NOW(), '".$uid."', '".$uid."', 'cr', '".$amount."', '".$closing_balance."', 'REFUND', '".$recharge_id."', '".$remark."', '0', '".$_SESSION['admin']."')");
						...
						if($ts1) {
							...
							$message = "Complaint Refund Successful, Txn: ".$recharge_id.", ".$operator_info->operator_name.", ".$complaint_info->account_no.", Rs.".$complaint_info->amount." Ur Bal: ".$closing_balance;
							smsSendSingle($user_info->mobile, $message, 'complaint_refund');
							echo "SUCCESS, Recharge refunded and Complaint has been closed successfully.";
							exit();
						} else {
							$db->query("ROLLBACK");
			...
			$message = "Recharge already successful, Txn: ".$recharge_id.", ".$operator_info->operator_name.", ".$complaint_info->account_no.", Rs.".$complaint_info->amount." Ref. Id: ".$operator_ref_no;
			smsSendSingle($user_info->mobile, $message, 'complaint_refund');
			echo "SUCCESS, Complaint has been closed successfully.";
			exit();
		} else if ($status == '3') {		
			$db->query("UPDATE complaints SET status = '1', refund_status = '".$status."', refund_by = '".$_SESSION['admin']."', refund_date = NOW(), remark = '".$remark."' WHERE complaint_id = '".$complaint_id."' ");
			$message = "Invalid Transaction, Txn: ".$recharge_id.", ".$operator_info->operator_name.", ".$complaint_info->account_no.", Rs.".$complaint_info->amount." ".$remark;
			smsSendSingle($user_info->mobile, $message, 'complaint_refund');
			echo "SUCCESS, Complaint has been closed successfully.";
			exit();
		} else if ($status == '4') {		
			$db->query("UPDATE complaints SET status = '1', refund_status = '".$status."', refund_by = '".$_SESSION['admin']."', refund_date = NOW(), remark = '".$remark."' WHERE complaint_id = '".$complaint_id."' ");
			$message = "Recharge already refunded, Txn: ".$recharge_id.", ".$operator_info->operator_name.", ".$complaint_info->account_no.", Rs.".$complaint_info->amount." ".$remark;
			smsSendSingle($user_info->mobile, $message, 'complaint_refund');
			echo "SUCCESS, Complaint has been closed successfully.";
			exit();
		} else {
