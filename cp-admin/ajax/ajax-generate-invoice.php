...
if($_POST['bill_type']!='' || $_POST['uid']!='' || $_POST['invoice_date']!='' || $_POST['bill_month']!='') {
	...
	if($_POST['bill_type']=='1' || $_POST['bill_type']=='3') {
		//
		$invs = $db->queryUniqueObject("SELECT * FROM gstinvoices WHERE bill_type!='2' ORDER BY id DESC ");
		$last_num = $invs->invoice_num;
		$new_num = ++$last_num;
		$invoice_num = sprintf('%06d', $new_num);
		//
	}
	$db->execute("INSERT INTO `gstinvoices`(`invoice_num`, `uid`, `bill_type`, `invoice_date`, `invoice_month`, `bill_mode`, `sub_total`, `tax_amount`, `total_amount`, `tax_type`, `create_date`) VALUES ('".$invoice_num."', '".$_POST['uid']."', '".$_POST['bill_type']."', '".$invoice_date."', '".$invoice_month."', '".$_POST['bill_mode']."', '".$_POST['sub_total']."', '".$_POST['tax_total']."', '".$_POST['total_amt']."', '".$_POST['tax_type']."', NOW())");
	...
	foreach($item as $key=>$val) {
		...
		$db->execute("INSERT INTO `gstinvoicesitems`(`invoiceid`, `uid`, `description`, `hsn_sac`, `amount`, `net_amount`, `taxable_amount`, `tax_amount`, `rate`, `unit`) VALUES ('".$invoiceid."', '".$_POST['uid']."', '".$item_desc."', '".$item_sac."', '".$item_amt."', '".$item_net."', '".$item_taxable."', '".$item_tax."', '".$item_rate."', '".$item_unit."')");
	}
	echo $invoiceid;
} else {
	echo 0;
