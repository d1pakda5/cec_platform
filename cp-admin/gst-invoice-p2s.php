...
$type = isset($_GET['type']) && $_GET['type']!='' ? mysql_real_escape_string($_GET['type']) : '3';
...
<head>
<title>Surcharge-Invoice-<?php echo $uid;?></title>
	...
	<div class="container">
		<div id="divPrint">
			...
			<table class="table table-bordered">
				...
				<?php
				$statement1 = "SELECT SUM(rch.amount) AS amt, SUM(rch.surcharge) AS sur_amt, rch.operator_id, opr.operator_name, opr.service_type, opr.hsn_sac_code, opr.item_group FROM apps_recharge rch LEFT JOIN operators opr ON rch.operator_id=opr.operator_id $sWhere AND item_group='4' GROUP BY rch.operator_id";
				$item1 = $gst->getRowByItemGroupSurcharge($statement1,$comms,$user->user_type);
				$item1_desc = $gst->getItemGroupName('4');
				$item1_sac = $gst->getItemGroupSac('4');
				if($item1['taxable']!=0 && $item1['gross']!=0) {
				...
				<?php
				$statement2 = "SELECT SUM(rch.amount) AS amt, SUM(rch.surcharge) AS sur_amt, rch.operator_id, opr.operator_name, opr.service_type, opr.hsn_sac_code, opr.item_group FROM apps_recharge rch LEFT JOIN operators opr ON rch.operator_id=opr.operator_id $sWhere AND item_group='5' GROUP BY rch.operator_id";
				$item2 = $gst->getRowByItemGroupSurcharge($statement2,$comms,$user->user_type);
				$item2_desc = $gst->getItemGroupName('5');
				$item2_sac = $gst->getItemGroupSac('5');
				if($item2['taxable']!=0 && $item2['gross']!=0) {
				...
				<?php
				$total_amount = $item1['sur'] + $item2['sur'];
				$total_taxable_amount = $item1['taxable'] + $item2['taxable'];
				$gst_sum = $gst->getTaxSummary($total_amount,$user->gst_type);
			...
			<table class="table table-bordered">
				...
				<?php
				$gst_sum1 = $gst->getTaxSummary($item1['sur'],$user->gst_type);
				$tax_total1 = $gst_sum1['cgst_amount']+$gst_sum1['sgst_amount']+$gst_sum1['igst_amount'];
				?>
				<tr>
					<td><?php echo $item1_sac;?></td>
					<td align="right"><?php echo $item1['taxable'];?></td>
					<td><?php echo $gst_sum1['cgst_rate'];?></td>
					<td align="right"><?php echo $gst_sum1['cgst_amount'];?></td>
					<td><?php echo $gst_sum1['sgst_rate'];?></td>
					<td align="right"><?php echo $gst_sum1['sgst_amount'];?></td>
					<td><?php echo $gst_sum1['igst_rate'];?></td>
					<td align="right"><?php echo $gst_sum1['igst_amount'];?></td>
					<td align="right"><?php echo $tax_total1;?></td>
				</tr>				
				<?php
				$gst_sum2 = $gst->getTaxSummary($item2['sur'],$user->gst_type);
				$tax_total2 = $gst_sum2['cgst_amount']+$gst_sum2['sgst_amount']+$gst_sum2['igst_amount'];
				?>
				<tr>
					<td><?php echo $item2_sac;?></td>
					<td align="right"><?php echo $item2['taxable'];?></td>
					<td><?php echo $gst_sum2['cgst_rate'];?></td>
					<td align="right"><?php echo $gst_sum2['cgst_amount'];?></td>
					<td><?php echo $gst_sum2['sgst_rate'];?></td>
					<td align="right"><?php echo $gst_sum2['sgst_amount'];?></td>
					<td><?php echo $gst_sum2['igst_rate'];?></td>
					<td align="right"><?php echo $gst_sum2['igst_amount'];?></td>
					<td align="right"><?php echo $tax_total2;?></td>
				</tr>
				<tr style="font-weight: bold">
