...
$sWhere = "WHERE rch.request_date BETWEEN '".$aFrom."' AND '".$aTo."' AND rch.status IN (0,1) AND usr.user_type='5' ";
...
if(isset($_GET['uid']) && $_GET['uid']!='') {
	$sWhere .= " AND dst.uid='".mysql_real_escape_string($_GET['uid'])."' ";
}
if(isset($_GET['type']) && $_GET['type']!='') {
	$sWhere .= " AND opr.billing_type='".mysql_real_escape_string($_GET['type'])."' ";
}
$statement = "apps_recharge rch LEFT JOIN operators opr ON rch.operator_id=opr.operator_id LEFT JOIN apps_user usr ON rch.uid=usr.uid LEFT JOIN usercommissions ucm ON usr.dist_id=ucm.uid AND rch.operator_id=ucm.operator_id LEFT JOIN apps_user dst ON usr.dist_id=dst.uid $sWhere ORDER BY rch.request_date ASC";
...
$self = $tbl->remove_page_param('rpt-distributor-gst-recharge-wise.php');
		...
		<div class="box">
			...
			<div class="box-body no-padding min-height-480">
					...
					<form method="get" id="rptRecharge" class="">
						...
						<div class="col-sm-2">
							<div class="form-group">
								...
								<button type="button" onclick="doExcel('rptGstRechargeWise')" class="btn btn-sm btn-success"><i class="fa fa-file-excel-o"></i> Excel</button>
							</div>
						</div>
					</form>
				</div>
				<table class="table table-condensed table-striped table-bordered">
					...
					<tbody>
						<?php
						$query = $db->query("SELECT rch.*, opr.operator_name, opr.billing_type, opr.is_surcharge, dst.uid AS myuid, dst.user_type, dst.company_name, dst.states, ucm.comm_dist, ucm.comm_api FROM {$statement} LIMIT {$startpoint}, {$limit} ");
						...
						<tr>
							...
							<td><?php echo $result->company_name;?> (<?php echo $result->myuid;?>)</td>
