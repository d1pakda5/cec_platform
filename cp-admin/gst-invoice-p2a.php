...
<div class="header">
	<div class="container-fluid text-right">
		...
		<div class="btn-group btn-group-sm hidden-print">
			...
			<a href="../gst/getpdf.php?id=<?php echo $invoice_num;?>&type=<?php echo $type;?>" class="btn btn-default"><i class="fa fa-download"></i> Download</a>
		</div>
		<?php }else{ ?>
		<div class="btn-group btn-group-sm hidden-print">
	...
	<div class="container">
		<div id="divPrint">
			...
			<table class="table table-bordered">
				...
				<?php
				$statement1 = "SELECT SUM(rch.amount) AS amt, rch.operator_id, opr.operator_name, opr.service_type, opr.hsn_sac_code, opr.item_group FROM apps_recharge rch LEFT JOIN operators opr ON rch.operator_id=opr.operator_id $sWhere AND item_group='3' GROUP BY rch.operator_id";
				$item1 = $gst->getRowByItemGroup($statement1,$comms,$user->user_type);
				$item1_desc = $gst->getItemGroupName('3');
				$item1_sac = $gst->getItemGroupSac('3');
				$gst_sum = $gst->getTaxSummary($item1['com'],$user->gst_type);
				$gross_total = $item1['com'];
				$total_net_amount = $item1['com'] - ($gst_sum['cgst_amount']+$gst_sum['sgst_amount']+$gst_sum['igst_amount']);
				$tax_total = $gst_sum['cgst_amount']+$gst_sum['sgst_amount']+$gst_sum['igst_amount'];
				$total_taxable_amount = $item1['com'] - ($gst_sum['cgst_amount']+$gst_sum['sgst_amount']+$gst_sum['igst_amount']);
				?>
				<tr>
					<td>1.
						...
						<input type="hidden" name="item_net[]" id="item_net" value="<?php echo $gross_total;?>">
						<input type="hidden" name="item_taxable[]" id="item_taxable" value="<?php echo $total_taxable_amount;?>">
						<input type="hidden" name="item_tax[]" id="item_tax" value="<?php echo $tax_total;?>">
					...
					<td align="right" class="tot_netamt"><?php echo $total_net_amount;?></td>
				</tr>
				<tr style="font-weight: bold">
