...
jQuery(document).ready(function(){
				...
				jQuery.ajax({ 
					...
					success	: function(data) {
						...
						if(obj["statuscode"]=='TXN') {
							jQuery.get("dmt3/list-beneficiary.php",{js_data: data, ajax: 'true'}, function(jd){
							...
							jQuery("#smobile").val(obj["RQST_MOBILE"]);
		...
		if(mobile=="" || fname=="" || lname=="") {
			...
			if(parseInt(mobile.length)!='10') {
				...
				jQuery.ajax({ 
					url : "dmt3-process.php",
					type : "GET",
					data : "request=senderRegistration&mobile="+mobile+"&fname="+fname+"&lname="+lname+"&pin="+pin,
					async : false,
					success	: function(data) {
						var obj = jQuery.parseJSON(data);
						if(obj["statuscode"]=='TXN') {							
						swal({
                    			title: "Success",
                    			text: "Remitter Registration Successfull ",
                    			type: "success",
	...
	jQuery("#submitBeneficiaryRegister").click(function() {
		var mobile = jQuery('#mobile').val().trim();
		var ben_fname = jQuery('#ben_fname').val().trim();
		var ben_lname = jQuery('#ben_lname').val().trim();
		var ben_account = jQuery('#ben_account').val().trim();
		var ifsc = jQuery('#ben_ifsc').val().trim();
		var remId=jQuery('#remId').text().trim();
		if(mobile=="" || ben_fname=="" ||  ben_lname=="" ||  remId=="" || ben_account=="" || ifsc=="") {
			...
			if(parseInt(mobile.length)!='10') {
				...
				jQuery.ajax({ 
					url : "dmt3-process.php",
					type : "GET",
					data : "request=beneficiaryAdd&mobile="+mobile+"&ben_fname="+ben_fname+"&ben_lname="+ben_lname+"&ben_account="+ben_account+"&ifsc="+ifsc+"&remId="+remId,
					async : false,
					success	: function(data) {
					   //alert(data);
					    var obj = jQuery.parseJSON(data);
					    if(obj["statuscode"]=='TXN') {	
							...
							swal.close();
							jQuery('#remIdotc').val(obj['data']['remitter']['id']);
						jQuery('#benId').val(obj['data']['beneficiary']['id']);
						jQuery('#otc_for').val('beneficiary');
	...
	jQuery("#submitOtcConfirm").click(function() {
		var mobile = jQuery('#mobile').val().trim();
		var remId = jQuery('#remIdotc').val().trim();
		var benId = jQuery('#benId').val().trim();
		var otc_code = jQuery('#otc_code').val().trim();
		var otc_for = jQuery('#otc_for').val().trim();
		if(mobile=="" || benId=="" || otc_code=="") {
			...
			if(parseInt(mobile.length)!='10') {
				...
				jQuery.ajax({ 
					url		: "dmt3-process.php",
					type	: "GET",
					data	: "request=otcConfirm&mobile="+mobile+"&benId="+benId+"&remId="+remId+"&otc="+otc_code+"&otc_for=beneficiary",
					async : false,
					success	: function(data) {
						var obj = jQuery.parseJSON(data);
						if(obj["statuscode"]=='TXN') {						
							swal({ title: "Success", text: obj["status"], type: "success"});
				...
				jQuery.ajax({ 
					url		: "dmt3-process.php",
					type	: "GET",
					data	: "request=otcConfirm&mobile="+mobile+"&benId="+benId+"&remId="+remId+"&otc="+otc_code+"&otc_for=bendelete",
					async : false,
					success	: function(data) {
						var obj = jQuery.parseJSON(data);
						if(obj["statuscode"]=='TXN') {						
	...
	jQuery("#submitMoneyRemittance").click(function() {		
		...
		if(mobile=="" || mr_uid=="" || mr_ben_name=="" || mr_ben_code=="" || mr_ben_account=="" || mr_ben_ifsc=="" ||  mr_amount=="") {
			swal({ title: "Error", text: "Mobile/Bank/Branch cannot be blank", type: "error"});
			return false;
		} else {
			if(parseInt(mobile.length)!='10') {
				...
				jQuery.ajax({ 
					url		: "dmt3-process.php",
					type	: "GET",
					data	: "request=moneyRemittance&mobile="+mobile+"&mr_uid="+mr_uid+"&mr_ben_name="+mr_ben_name+"&mr_ben_code="+mr_ben_code+"&mr_ben_account="+mr_ben_account+"&mr_ben_ifsc="+mr_ben_ifsc+"&mr_amount="+mr_amount,
					async : false,
					success	: function(data) {
...
function benDelete(benId,remId, uid) {
	var mobile = jQuery('#mobile').val().trim();
	if(mobile=="" || benId=="" || remId=="") {
		swal({ title: "Error", text: "Mobile number cannot be blank", type: "error"});
	} else {
		swal({ title: "Processing", text: "Please wait a moment..", imageUrl: "../images/preloader.gif"});
		jQuery.ajax({ 
			url		: "dmt3-process.php",
			type	: "GET",
			data	: "request=beneficiaryDelete&mobile="+mobile+"&benId="+benId+"&remId="+remId,
			async : false,
			success	: function(data) {
				var obj = jQuery.parseJSON(data);
				console.log(obj);
				if(obj["statuscode"]=='TXN') {
					swal.close();
					jQuery('#remIdotc').val(remId);
					jQuery('#benId').val(benId);
					jQuery('#otc_for').val('bendelete');
					jQuery('#modalOtcConfirm').modal('show');
				} else if (obj["RESP_CODE"]=='1') {
					swal({ title: "Error", text: obj["Message"], type: "error"});
				} else {
...
function benValidate(benId, ben_ifsc, ben_account, uid) {
	var mobile = jQuery('#mobile').val().trim();
	if(mobile=="" || benId=="" || ben_account=="" || ben_ifsc=="" || uid=="" || remId=="") {
		swal({ title: "Error", text: "Mobile number cannot be blank", type: "error"});
	} else {
		swal({
			title: "Are you sure?",
		...
		function(){
			...
			jQuery.ajax({ 
				url		: "dmt3-process.php",
				type	: "GET",
				data	: "request=beneficiaryValidate&mobile="+mobile+"&ben_account="+ben_account+"&ifsc="+ben_ifsc+"&benId="+benId+"&uid="+uid,
				async : false,
				success	: function(data) {
					var obj = jQuery.parseJSON(data);
					//console.log(obj);
...
function benRemittance(benName,benId, ben_account, ben_ifsc, uid) {
	jQuery('#mr_ben_name').val(benName);
	jQuery('#mr_ben_code').val(benId);
						...
						<div class="box-body">	
							<form id="frmSenderRegister" class="form-horizontal">
								...
								<div class="form-group">
									...
									<div class="col-sm-5">
										<input type="text" name="pin" id="pin" class="form-control" placeholder="Enter PinCode" />
									</div>
								</div>
								<div class="form-group">
				...
				<div id="boxBeneficiaryForm" style="display:none;">
					<div class="dmt box">
						...
						<div class="box-body">	
							<form id="frmBeneficiaryRegister" class="form-horizontal">
								<div class="form-group">
									<label class="col-sm-3 control-label">Beneficiary First Name :</label>
									<div class="col-sm-5">
										<input type="text" name="ben_fname" id="ben_fname" class="form-control" placeholder="Enter Beneficiary First Name" />
									</div>
								</div>
								<div class="form-group">
									<label class="col-sm-3 control-label">Beneficiary Last Name :</label>
									<div class="col-sm-5">
										<input type="text" name="ben_lname" id="ben_lname" class="form-control" placeholder="Enter Beneficiary Last Name" />
									</div>
								</div>
								<div class="form-group">
									<label class="col-sm-3 control-label">Bank Account :</label>
				...
				<div class="row">
					<div class="col-sm-12">
						<form id="frmOtcConfirm" class="form-horizontal">
						    <div class="form-group">
								<label class="col-sm-4 control-label">Remitter ID:</label>
								<div class="col-sm-6">
									<input type="text" name="remIdotc" id="remIdotc" readonly="" class="form-control" placeholder="Enter Reference" />
								</div>
							</div>
							<div class="form-group">
								<label class="col-sm-4 control-label">Beneficiary ID :</label>
								<div class="col-sm-6">
									<input type="text" name="benId" id="benId" readonly="" class="form-control" placeholder="Enter Reference" />
								</div>
							</div>
							<div class="form-group">
								...
								<div class="col-sm-6">
									<input type="text" name="otc_code" id="otc_code" class="form-control" placeholder="Enter OTC" />
										<input type="hidden" name="otc_for" id="otc_for" class="form-control" placeholder="Enter OTC" />
								</div>
							</div>
						</form>
					</div>
