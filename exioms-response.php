...
if(isset($_GET['transtype']) && $_GET['transtype'] != '' && isset($_GET['accountid']) && $_GET['accountid'] != '') {
	...
	$recharge_id = trim(mysql_real_escape_string($_GET['accountid']));
	
	$recharge_info = $db->queryUniqueObject("SELECT * FROM apps_recharge WHERE recharge_id = '".$recharge_id."' AND api_id = '7' ");
	if($recharge_info) {
