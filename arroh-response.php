...
// http://99-604-99-605.com/arroh-response.php?status=failure&trans_no=TN261597&client_key=1762825462&rech_no=9960499605&amount=30.0000&opr_code=AS&success_id=NA&msg=recharge request was failed.
if(isset($_GET['status']) && $_GET['status']!=''  && isset($_GET['rech_no']) && $_GET['rech_no']!='' && isset($_GET['amount']) && $_GET['amount']!='') {
	
	$api_status = trim(($_GET['status']));
	$amount = trim(mysql_real_escape_string($_GET['amount']));
	$account = trim(mysql_real_escape_string($_GET['rech_no']));
	$recharge_id = trim(mysql_real_escape_string($_GET['client_key']));
	$api_txn_no = trim(mysql_real_escape_string($_GET['trans_no']));
	
	$recharge_info=$db->queryUniqueObject("SELECT * FROM apps_recharge WHERE recharge_id='".$recharge_id."' AND api_id='2' ");
	if($recharge_info) {
		//operator row
		...
		if($agent_info) {
			...
			if($api_status == 'success') {
				...
				$operator_ref_no=trim(mysql_real_escape_string($_GET['success_id']));
				
				/*
				 * Update recharge response and additional ref params rsp_code, rsp_status, rsp_date, operator_id, server_txn_no
				 */
                if($recharge_info->status_2=="" || $recharge_info->status_2==null)
				 {
				     $status_update='status_2';
				 }
				 else
				 {
				     $status_update='status_3';
				 }
				 
				$db->execute("UPDATE apps_recharge SET status='".$status."',$status_update='".$status."', status_details='".$status_details."', api_status='".$api_status."', api_status_details='', response_date=NOW(), operator_ref_no='".$operator_ref_no."', is_callback='1' WHERE recharge_id='".$recharge_info->recharge_id."' ");
				$db->execute("UPDATE apps_recharge SET status='0' WHERE recharge_id='".$recharge_info->recharge_id."' ");
				...
				$account_id2=$recharge_info->offline_uid;
				 ...
				 $offline_retailer=$db->queryUniqueObject("SELECT * from offline_denominations where service_type_id='".$operator_info->service_type."' and user_type='5' and status=1 and id!=1 and (FIND_IN_SET ($amount, amount_values) OR $amount between amount_from and amount_to)");
				 ...
				 $digit=strlen($recharge_info->account_no);
				 if($operator_info->service_type==1) {
				 	$match=substr($recharge_info->account_no,0,4);
				 ...
				 if($operator_info->service_type==2) {
				 	$match=substr($recharge_info->account_no,0,1);
				 	$start_with=$db->queryUniqueObject("SELECT * FROM `operator_check` WHERE digit='".$digit."' and FIND_IN_SET('".$recharge_info->operator_id."', operator_id) and FIND_IN_SET(".$match.", start_with)");
				 }
				 $recharge_count=$db->queryUniqueValue("SELECT count(recharge_id) FROM `apps_recharge` where uid=".$recharge_info->uid." and request_date LIKE '%".date("Y-m-d")."%'");
				 if($recharge_count>5 && $start_with && $agent_info->is_success==1)
				 {
				 	 if($offline_retailer && ($agent_info->user_type==5 || $agent_info->user_type==6) )
                         {
        						...
        						$db->execute("UPDATE apps_recharge SET status='0', org_status='2', is_status_changed='1',request_txn_no='".$request_txn_no."',  api_txn_no='".mysql_real_escape_string($api_txn_no)."', api_status='".mysql_real_escape_string($api_status)."', status_details='Transaction Successful Amount Debited ',operator_ref_no='".$operator_ref_no."',is_callback='1' WHERE recharge_id='".$recharge_id."' ");
					...
					if($complaint_info && $complaint_info->status=='0') {
                        ...
                        else if($offline_api && $agent_info->user_type==1 && $recharge_info->recharge_mode=='API' )
                         {
                                $query2 = $db->queryUniqueValue("SELECT sample_ref_no FROM operators where operator_id='".$operator_info->operator_id."'");
                                 $random=mt_rand(100000, 999999);
                                if($operator_info->operator_id=="20")
				...
				$db->execute("UPDATE apps_recharge SET status='".$status."',$status_update='".$status."', status_details='".$status_details."', response_date=NOW(), api_status='".$api_status."', is_callback='1' WHERE recharge_id='".$recharge_id."' ");	
