     ...
     if($is_money!=i)
    	...
    	if($operator_info) {		
    		...
    		if($hasFund && ($hasFund->balance >= $amount)) {
    			...
    			//SEND TO CHECK API
    			include(DIR."/dmt3/beneficiary-validate-check.php");
    			//
    			if(isset($dmCheckStatus) && $dmCheckStatus=="0") {
    				...
    				if($wallet && $ts1) {
    					...
    					if($commit) {
    						$remark = "DMT3 VALIDATE: $recharge_id, $account, $amount, $debit_amount";
    						...
    						//Send to payment api
    						include(DIR."/dmt3/beneficiary-validate-pay.php");
    						...
    						if($status=='0') {
    							...
    							if($wallet1 && $ts4) {													
    								$commit = $db->query("COMMIT");	
    								if($commit) {													
    									$remark_new = "DMT3 VALIDATE REVERT: $recharge_id, $account, $amount, $debit_amount, failed revert";
    			...
    			} else if(isset($dmCheckStatus) && $dmCheckStatus=="1") {
