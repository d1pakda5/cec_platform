...
$payExplode = explode("\n",$payOutput);
// print_r($payExplode);
// $db->execute("INSERT INTO `yb_logs`(`log_date`, `log_type`, `log_content`) VALUES (NOW(), 'YB: PAY REMITTANCE', '".$payOutput."')");
...
if($dmPayError=='0' && $dmPayResult=='0') {
	...
	if(isset($dmPayAddInfo) && $dmPayAddInfo=='') {
		$jsFetch = urldecode($dmPayAddInfo);
		$jsDmtResponse = '{"RESP_CODE":"300","RESP_MSG":"Transaction Successful"}';	
