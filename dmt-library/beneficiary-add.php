...
if(strpos($checkExplode['6'], 'ERROR=0')!==false) {
	...
	if(strpos($payExplode['6'], 'ERROR=0')!==false) {
		...
		$jsResponse = '{"ResponseCode":"0","Message":"Verification code for beneficary add is sent on requested mobile.","RequestNo":"'.$jsFetch['RequestNo'].'","MobileNo":"'.$jsFetch['MobileNo'].'"}';		
		echo $jsResponse;
		exit();
